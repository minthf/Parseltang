...
class Koh_curve:
	...
	def draw_koh_curve(self, start: Point, end: Point, deep=3):
		if deep == 1:
			return
		...
		self.draw_koh_curve(start, second_point, deep - 1)
		self.draw_koh_curve(second_point, third_point, deep - 1)
		self.draw_koh_curve(third_point, fourth_point, deep - 1)
		self.draw_koh_curve(fourth_point, end, deep - 1)


start_point = Point(10, 200)
end_point = Point(390, 200)

koh = Koh_curve(start_point,end_point)
draw.line((start_point.x, start_point.y, end_point.x, end_point.y))

koh.draw_koh_curve(koh.start, koh.end, 5)
